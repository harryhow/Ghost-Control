...
class MoveMouse():
    ...
    def typenrun(self):
        ...
        #type lines
        with open('CircleGradientMain.txt') as f:
            snippet_a = f.readlines()

        try:
            firstline = snippet_a[0]
        except IndexError:
            firstline = 'null'
        ...
        if current_window == 0:
            ...
            for index, val in enumerate(snippet_a):
                print "typing x, y =",lineXLoc, lineYLoc
                # m.click(0.25 * x_dim, 0.25 * y_dim, 1)
                m.move(lineXLoc, lineYLoc)
                #k.press_key('function')
                k.type_string(val)
                lineYLoc = lineYLoc + 15 # line pixel?
                time.sleep(0.1)
                if lineYLoc > 450:
                    break;
        ...
        # with open ("CircleGradientMain.txt", "r") as myfile:
